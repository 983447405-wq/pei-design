...
export default async function AboutPage({ params }: Props) {
  ...
  const principles = [
    ...
    locale === "zh" ? "设计系统应该沉淀产品行为，而不只是视觉组件。" : "Design systems should codify product behavior, not just UI components.",
  ...
  return (
    <main>
      ...
      <Section eyebrow="Capability" title={locale === "zh" ? "我能进入模糊阶段，也能推动落地。" : "I can enter ambiguous stages and drive delivery."}>
        <div className="glass motion-card rounded-2xl p-6 md:p-8">
          {[
            ["Strategy", locale === "zh" ? "AI 产品机会识别、MVP 范围、体验原则、评测指标。" : "AI opportunity framing, MVP scope, UX principles, evaluation metrics."],
            ["UX / UI", locale === "zh" ? "信息架构、复杂工作流、设计系统、高保真原型。" : "Information architecture, complex workflows, design systems, high-fidelity prototypes."],
            ["Delivery", locale === "zh" ? "与产品、算法、工程对齐拆解，减少交付损耗。" : "Align with product, ML, and engineering to reduce delivery loss."]
          ].map(([title, body]) => (
